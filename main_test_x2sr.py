...
from models.network_x2sr import X2SR as net
...
def main():
    ...
    parser.add_argument('--training_patch_size', type=int, default=128, help='patch size used in training X2SR. '
                                       'Just used to differentiate two different settings in Table 2 of the paper. '
                                       'Images are NOT tested patch by patch.')
    parser.add_argument('--large_model', action='store_true', help='use large model, only provided for real image sr')
    parser.add_argument('--model_path', type=str,
                        default='model_zoo/x2sr/X2SR_ClassicalSR_X2_64.pth')
    ...
    if os.path.exists(args.model_path):
        ...
        url = 'https://github.com/mv-lab/x2sr/releases/download/v0.0.1/{}'.format(os.path.basename(args.model_path))
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        cv2.imwrite(f'{save_dir}/{imgname}_X2SR.png', output)
...
def setup(args):
    ...
    if args.task in ['classical_sr', 'lightweight_sr', 'compressed_sr']:
        save_dir = f'results/x2sr_{args.task}_x{args.scale}'
    ...
    # 006 JPEG compression artifact reduction
        ...
        save_dir = f'results/x2sr_{args.task}_jpeg{args.jpeg}'
        folder = args.folder_gt
        border = 0
        window_size = 7
